...
const AvatarBadge = ({color}) => {
  return (
    <div className={`bg-${color}-400 rounded-full w-4 h-4 borderc border-2 border-white absolute bottom-0 right-0`}>
    </div>
  )
}
...
const Avatar = (
  ...
  if(src) {
    return (
      <div className={ avatarStyle({size}) + " " + className}>
    ...
    return (
      <div className={ avatarStyle({size}) + " " + className}>
        <svg className={ avatarStyle({size}) + " fill-current " + className}  viewBox="0 0 128 128" role="img">
          <g>
